...
@php
    ...
    $namaUnitKerja = $unitKerja?->nama ?? 'Instansi';

    $dokumenMap = $pengadaan->dokumenPengadaan
        ? $pengadaan->dokumenPengadaan->keyBy('jenis')
        : collect();

    $bapmhp = $dokumenMap->get('bapmhp') ?? $dokumen ?? null;

    $tanggalBapmhpRaw = $bapmhp?->tanggal
    ...
    $tanggalCarbon = \Carbon\Carbon::parse($tanggalBapmhpRaw);
...
<div class="paragraph">
    ...
    yang selanjutnya disebut <span class="font-bold">PIHAK KESATU</span> sebagai yang menyerahkan
    hasil Pekerjaan Pengadaan {{ $namaRekening }}.
</div>
...
<div class="paragraph">
    ...
    hasil pekerjaan dalam kondisi sesuai, dengan uraian sebagaimana terlampir sesuai dengan
    Surat Pesanan/SP Nomor: {{ $pengadaan->no_kontrak ?? '-' }} tanggal {{ $tanggalKontrak }}.
</div>

<div class="paragraph">
    Demikian Berita Acara Pemeriksaan Hasil Pekerjaan {{ $namaRekening }} dalam rangkap
    3 (Tiga) untuk dipergunakan sebagaimana mestinya.
</div>
