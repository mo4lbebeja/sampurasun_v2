...
// ── PERUBAHAN 1: tambah prop usulan dan catatanRevisi ──────────────
    ...
    defineProps<{
        ...
        usulan: {
            ...
            items: Array<{
                ...
                satuan: string;
                jumlah: number;
                harga_satuan_estimasi: number | string;
            }>;
        };
...
// ── PERUBAHAN 2: form diisi dari data usulan yang ada ──────────────

const form = useForm({
    judul:          props.usulan.judul,
    latar_belakang: props.usulan.latar_belakang ?? '',
    keterangan:     props.usulan.keterangan ?? '',
    anggaran_id:    props.usulan.anggaran_id as number | null,
    items: props.usulan.items.map((item) => ({
...
const kategoriName = (kategoriId: number | null) => {
    ...
    return kategoriList.value.find((k) => k.id === kategoriId)?.nama ?? '-';
};
...
// ── PERUBAHAN 3: submit PUT bukan POST ─────────────────────────────
...
const submit = () => {
    ...
    // PUT /usulan/{id} → UsulanPengadaanController::update()
    form.put(`/usulan/${props.usulan.id}`, { preserveScroll: true });
};

const satuanOptions = [
    'unit', 'pcs', 'buah', 'paket', 'set', 'box', 'rim', 'lusin',
    'meter', 'roll', 'liter', 'kg', 'gram', 'bulan', 'tahun', 'layanan', 'lainnya',
];
...
<template>
    <!-- PERUBAHAN 4: judul halaman -->
    <Head :title="`Perbaiki ${usulan.no_usulan}`" />

    <form @submit.prevent="submit" class="space-y-6 p-4 sm:p-6 lg:p-8">

        <!-- Header dengan tombol back -->
        <div class="flex items-start gap-3">
            <!-- PERUBAHAN 5: back ke detail usulan, bukan ke /usulan -->
            <Link
            ...
            <div class="flex-1">
                <!-- PERUBAHAN 6: judul dan subtitle berbeda -->
                <PageHeader
                    title="Perbaiki Usulan"
                    :subtitle="usulan.no_usulan"
                    eyebrow="Revisi"
                />
            </div>
        </div>

        <!-- PERUBAHAN 7: banner catatan revisi dari PPTK -->
        <div
            v-if="catatanRevisi"
            class="flex items-start gap-4 rounded-lg border border-amber-300 bg-amber-50 p-5 dark:border-amber-700 dark:bg-amber-950"
        >
            <RotateCcw class="mt-0.5 h-5 w-5 shrink-0 text-amber-600 dark:text-amber-400" />
            <div>
                <p class="text-sm font-semibold text-amber-800 dark:text-amber-300">
                    Catatan dari PPTK — Diminta Revisi
                </p>
        ...
        <!-- Section: Anggaran — SAMA PERSIS dengan Create.vue -->
                    ...
                    <div>
                        ...
                        >
                            ...
                            <div class="mt-1 space-y-1 text-xs">
                                <div>Sisa Anggaran: <span class="font-mono font-semibold">{{ formatRupiah(sisaAnggaran) }}</span></div>
                                <div>Total Estimasi: <span class="font-mono font-semibold">{{ formatRupiah(totalEstimasi) }}</span></div>
                                <div v-if="isEstimasiMelebihiSisa">Kelebihan: <span class="font-mono font-semibold">{{ formatRupiah(selisihEstimasi) }}</span></div>
                            </div>
        ...
        <!-- Section: Info Usulan — SAMA PERSIS dengan Create.vue -->
        ...
        <!-- Section: Rincian Barang — SAMA PERSIS dengan Create.vue -->
        <Section title="Rincian Barang" eyebrow="Langkah 3" :description="`${form.items.length} item`">
            <template #actions>
                <PrimaryButton type="button" variant="secondary" size="sm" @click="openCreateItem">
            ...
            >
                ...
                <p class="mt-1 text-sm text-muted-foreground">Tambahkan barang atau jasa yang akan diusulkan.</p>
                <PrimaryButton type="button" variant="primary" class="mt-4" @click="openCreateItem">
                    <Plus class="h-4 w-4" />
                    Tambah Item Pertama
                </PrimaryButton>
            </div>
                ...
                <div class="max-h-[420px] overflow-auto">
                    <table class="w-full min-w-[900px] text-sm">
                        ...
                        <tbody>
                            ...
                            >
                                <td class="px-3 py-3 align-top font-mono text-xs text-muted-foreground">{{ idx + 1 }}</td>
                                <td class="px-3 py-3 align-top">
                                    <div class="font-medium text-foreground">{{ item.nama_barang || '-' }}</div>
                                    <div v-if="item.spesifikasi" class="mt-0.5 line-clamp-2 text-xs text-muted-foreground">{{ item.spesifikasi }}</div>
                                    <div v-if="hasItemError(idx)" class="mt-1 text-xs font-medium text-destructive">Ada data item yang perlu diperbaiki.</div>
                                </td>
                                <td class="px-3 py-3 align-top">{{ kategoriName(item.kategori_id) }}</td>
                                <td class="px-3 py-3 text-right align-top font-mono">{{ Number(item.jumlah).toLocaleString('id-ID') }}</td>
                                <td class="px-3 py-3 align-top">{{ item.satuan }}</td>
                                <td class="px-3 py-3 text-right align-top font-mono">{{ formatRupiah(item.harga_satuan_estimasi) }}</td>
                                <td class="px-3 py-3 text-right align-top font-mono font-semibold">{{ formatRupiah(subtotal(item)) }}</td>
                                <td class="px-3 py-3 align-top">
                                    <div class="flex justify-end gap-2">
                                        <button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-border bg-background transition hover:bg-muted" title="Edit item" @click="openEditItem(idx)">
                                            <Pencil class="h-3.5 w-3.5" />
                                        </button>
                                        <button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-border bg-background transition hover:bg-muted" title="Duplikat item" @click="duplicateItem(idx)">
                                            <Copy class="h-3.5 w-3.5" />
                                        </button>
                                        <button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-border bg-background transition hover:bg-destructive/10" title="Hapus item" style="color: var(--color-brand-danger);" @click="removeItem(idx)">
            ...
            <div class="mt-4 flex flex-col gap-3 rounded-md border border-primary/30 bg-primary/5 p-5 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <div class="mt-0.5 text-xs text-muted-foreground">{{ form.items.length }} item pengadaan</div>
                </div>
                <div class="text-left sm:text-right">
                    <div class="text-xs text-muted-foreground">Total Estimasi</div>
                    <span class="font-display text-2xl font-bold text-primary">{{ formatRupiah(totalEstimasi) }}</span>
                </div>
            </div>
        </Section>

        <!-- Modal Tambah/Edit Item — SAMA PERSIS dengan Create.vue -->
        <div
            v-if="itemModalOpen"
        ...
        >
            <div class="w-full max-w-3xl overflow-hidden rounded-xl border border-border bg-card shadow-xl">
                <div class="flex items-center justify-between border-b border-border px-5 py-4">
                    <div>
                        <div class="text-eyebrow">{{ editingIndex === null ? 'Tambah Item' : 'Edit Item' }}</div>
                        <h3 class="font-display text-lg font-semibold">Rincian Barang/Jasa</h3>
                    </div>
                    <button type="button" class="inline-flex h-9 w-9 items-center justify-center rounded-md border border-border bg-background transition hover:bg-muted" @click="closeItemModal">
                        <X class="h-4 w-4" />
                    </button>
                </div>
                ...
                <div class="max-h-[70vh] overflow-y-auto p-5">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            <label class="mb-1.5 block text-sm font-semibold">Kategori <span class="text-destructive">*</span></label>
                            <select v-model="itemDraft.kategori_id" class="h-11 w-full rounded-md border border-input bg-background px-3 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary">
                                <option :value="null">— Pilih kategori —</option>
                                <option v-for="k in kategoriList" :key="k.id" :value="k.id">{{ k.nama }}</option>
                            </select>
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-semibold">Nama Barang <span class="text-destructive">*</span></label>
                            <input v-model="itemDraft.nama_barang" type="text" placeholder="Nama barang/jasa" class="h-11 w-full rounded-md border border-input bg-background px-3 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
                        </div>
                        <div class="sm:col-span-2">
                            <label class="mb-1.5 block text-sm font-semibold">Spesifikasi</label>
                            <textarea v-model="itemDraft.spesifikasi" rows="3" placeholder="Spesifikasi teknis, merek, tipe, ukuran, atau catatan detail lainnya" class="w-full rounded-md border border-input bg-background px-3 py-2.5 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-semibold">Jumlah <span class="text-destructive">*</span></label>
                            <input v-model.number="itemDraft.jumlah" type="number" min="1" class="h-11 w-full rounded-md border border-input bg-background px-3 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-semibold">Satuan <span class="text-destructive">*</span></label>
                            <select v-model="itemDraft.satuan" class="h-11 w-full rounded-md border border-input bg-background px-3 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary">
                                <option v-for="satuan in satuanOptions" :key="satuan" :value="satuan">{{ satuan === 'lainnya' ? 'Lainnya...' : satuan }}</option>
                            </select>
                            <input v-if="itemDraft.satuan === 'lainnya'" v-model="customSatuan" type="text" placeholder="Tulis satuan lainnya, contoh: hari, orang, lot" class="mt-2 h-11 w-full rounded-md border border-input bg-background px-3 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
                            <p v-if="itemDraft.satuan === 'lainnya'" class="mt-1 text-xs text-muted-foreground">Satuan yang ditulis di sini akan disimpan ke item pengadaan.</p>
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-semibold">Harga Satuan Estimasi <span class="text-destructive">*</span></label>
                            <input v-model.number="itemDraft.harga_satuan_estimasi" type="number" min="0" class="h-11 w-full rounded-md border border-input bg-background px-3 text-right font-mono text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
                        </div>
                        <div class="rounded-md border border-dashed border-border bg-secondary/30 p-4">
                            <div class="text-eyebrow">Subtotal</div>
                            <div class="mt-1 font-display text-xl font-bold text-primary">{{ formatRupiah(itemDraftSubtotal) }}</div>
                        </div>
                    </div>
                </div>

                <div class="flex flex-col-reverse gap-3 border-t border-border px-5 py-4 sm:flex-row sm:justify-end">
                    <PrimaryButton type="button" variant="secondary" @click="closeItemModal">Batal</PrimaryButton>
                    <PrimaryButton type="button" variant="primary" :disabled="itemDraft.satuan === 'lainnya' && !customSatuan.trim()" @click="saveItem">
        ...
        <!-- Submit row — PERUBAHAN 8: label dan kondisi tombol berbeda -->
        <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            <Link :href="`/usulan/${usulan.id}`">
                <PrimaryButton variant="secondary" type="button">Batal</PrimaryButton>
            </Link>
            <PrimaryButton
                type="submit"
                variant="primary"
            ...
            >
                {{ form.processing ? 'Menyimpan...' : 'Simpan & Ajukan Ulang' }}
            </PrimaryButton>
        </div>
